...
- mail-admin(Go): JWT 로그인 + RBAC + 계정 CRUD + 감사 로그 + 매핑 파일 생성
...
## 3-1) 메일박스/메시지 아카이브 API (선택)

외부 수신/발신 메일을 DB에 저장하려면 아카이브 DB 옵션을 켭니다.
...
## 8) OAuth2 소셜 로그인 (Google / Microsoft)
